...
- Name: kanban.spec.ts >> KanbanLocal E2E Tests >> should allow task creation, updating task details, and global view list integration
- Location: tests\kanban.spec.ts:28:3
...
- complementary:
  ...
  - navigation:
    ...
    - button "Add Project"
    - link "E2E Project":
      - /url: "#/project/2f2d21e6-1825-4190-975a-e50556702564"
  - button "Settings"
- main:
  - heading "E2E Project" [level=2]
  - button "Edit Project"
...
```ts
  ...
  2   | 
  3   | test.describe('KanbanLocal E2E Tests', () => {
  4   |   
  5   |   test.beforeEach(async ({ page }) => {
  6   |     // Navigate to the root URL (configured base basename is /KanbanLocal/)
  7   |     await page.goto('/KanbanLocal/');
  8   |   });
  9   | 
  10  |   test('should display initial empty state and allow project creation', async ({ page }) => {
  11  |     // Ensure title or branding is present
  12  |     await expect(page.locator('aside')).toContainText('KanbanLocal');
  13  | 
  14  |     // Click on Add Project button
  15  |     await page.locator('button:has-text("Add Project")').first().click();
  16  | 
  17  |     // Fill project form
